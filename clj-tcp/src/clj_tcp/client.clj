...
(defn close-client [{:keys [^NioEventLoopGroup group ^NioEventLoopGroup read-group ^ChannelFuture channel-f internal-error-ch write-ch id
...
(defn blocking-write! [^Client client v]
      (if (open? client)
        (-> client ^ChannelFuture (:channel-f) .channel ^ChannelFuture (.writeAndFlush v) (.awaitUninterruptibly))
        (throw (RuntimeException. "Client closed"))))
      ...
      ([host port {:keys [group read-group
        ...
        (try
          (let [g (if group group EVENT-LOOP-GROUP)
               ...
               (let [ch-f (.connect b)
                     conn (->Client g read-group ch-f write-ch read-ch internal-error-ch error-ch reconnect-count closed -1)]
                    (.sync ch-f)
                    (info "Started client " host)
                    (assoc conn :id (mon/join-ctx :tcp-conn conn))))
          (catch Exception e (do
                               (error e e)
                               (>!! internal-error-ch [e nil])
                               nil
...
(defn client [host port {:keys [handlers
      ...
      (let [
           ...
           (if (not client)
             (do
               (let [cause (read-error {:error-ch internal-error-ch} 200)]
           ...
           (thread
             (try
                                          ...
                                          (let [c
                                                (loop [acc 0] ;reconnect in loop
                                                      (if (>= acc retry-limit)
                                                        ...
                                                        (let [v1
                                                              (try
                                                                (let [_ (close-all local-client) ;we must close before we start a new client
                                                                      c (start-client host port conf)
                                                                     ...
                                                                     (warn (str "Retry connection " host))
               ...
               (catch Exception e (do (error "Critical Errror - exception in read look") (error e e )))))
           ...
           (thread
             (try
               ...
               (catch Exception e (do (error "Critical error eception in read write-ch look ") (error e e)))))
